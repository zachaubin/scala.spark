...
object EmpiricalObservation2_1992_v2 {

  def main(args: Array[String]): Unit = {
//    val sc = SparkSession.builder().master("spark://richmond:30166").getOrCreate().sparkContext
         val sc = SparkSession.builder().master("local").getOrCreate().sparkContext
    sc.setLogLevel("ERROR")
    println("Starting EmpiricalObservation2_1992. If you see this something is running.")

    val g20_value = 727

    // (9212268,List(9304281, 9308201, 9304310, 9306321, 9310354, 9311259))
    var valid_node_neighbor_1992 = sc.textFile("./loaders/valid_node_neighbor_1992").
    ...
    var path_container = sc.textFile("./loaders/path_container_1992").map( x => (x.split(",")(0).
...
//      try {
//        ixx = i
//        lenNew = pathAdder(lenOld, valid_node_neighbor_1992)
//        checkIt(lenNew)
//        lenOld = lenNew
//      } catch {
//        case err: Any => {
...
//    }
    ...
    try {
      ...
      ixx = ixx + 1
      val len11 = pathAdder(len10, valid_node_neighbor_1992)
      checkIt(len11)

      ixx = ixx + 1
      val len12 = pathAdder(len11, valid_node_neighbor_1992)
      checkIt(len12)

      ixx = ixx + 1
      val len13 = pathAdder(len12, valid_node_neighbor_1992)
      checkIt(len13)

      ixx = ixx + 1
      val len14 = pathAdder(len13, valid_node_neighbor_1992)
      checkIt(len14)

      ixx = ixx + 1
      val len15 = pathAdder(len14, valid_node_neighbor_1992)
      checkIt(len15)

      ///////////////////////////////////////////////////

      ixx = ixx + 1
      val len16 = pathAdder(len15, valid_node_neighbor_1992)
      checkIt(len16)

      ixx = ixx + 1
      val len17 = pathAdder(len16, valid_node_neighbor_1992)
      checkIt(len17)

      ixx = ixx + 1
      val len18 = pathAdder(len17, valid_node_neighbor_1992)
      checkIt(len18)

      ixx = ixx + 1
      val len19 = pathAdder(len18, valid_node_neighbor_1992)
      checkIt(len19)

      ixx = ixx + 1
      val len20 = pathAdder(len19, valid_node_neighbor_1992)
      checkIt(len20)
    } catch {
