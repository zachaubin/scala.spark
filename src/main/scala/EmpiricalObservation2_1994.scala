...
object EmpiricalObservation2_1994 {

  def main(args: Array[String]): Unit = {
    val sc = SparkSession.builder().master("spark://richmond:30166").getOrCreate().sparkContext
    //     val sc = SparkSession.builder().master("local").getOrCreate().sparkContext
    sc.setLogLevel("ERROR")
    println("Starting EmpiricalObservation2_1994. If you see this something is running.")

    val g20_value = 6685253

    // (9212268,List(9304281, 9308201, 9304310, 9306321, 9310354, 9311259))
    var valid_node_neighbor_1994 = sc.textFile("/loaders/valid_node_neighbor_1994").
    ...
    var path_container = sc.textFile("/loaders/path_container_1994").map( x => (x.split(",")(0).
...
//      try {
//        ixx = i
//        lenNew = pathAdder(lenOld, valid_node_neighbor_1994)
//        checkIt(lenNew)
//        lenOld = lenNew
//      } catch {
//        case err: Any => {
...
//    }
    ...
    try {
      ...
      val len4 = pathAdder(len3, valid_node_neighbor_1994)
      checkIt(len4)


      ixx = ixx + 1
      val len5 = pathAdder(len4, valid_node_neighbor_1994)
      checkIt(len5)

      ///////////////////////////////////////////////////

      ixx = ixx + 1
      val len6 = pathAdder(len5, valid_node_neighbor_1994)
      checkIt(len6)

      ixx = ixx + 1
      val len7 = pathAdder(len6, valid_node_neighbor_1994)
      checkIt(len7)

      ixx = ixx + 1
      val len8 = pathAdder(len7, valid_node_neighbor_1994)
      checkIt(len8)

      ixx = ixx + 1
      val len9 = pathAdder(len8, valid_node_neighbor_1994)
      checkIt(len9)

      ixx = ixx + 1
      val len10 = pathAdder(len9, valid_node_neighbor_1994)
      checkIt(len10)
      ...
      ixx = ixx + 1
      val len11 = pathAdder(len10, valid_node_neighbor_1994)
      checkIt(len11)

      ixx = ixx + 1
      val len12 = pathAdder(len11, valid_node_neighbor_1994)
      checkIt(len12)

      ixx = ixx + 1
      val len13 = pathAdder(len12, valid_node_neighbor_1994)
      checkIt(len13)

      ixx = ixx + 1
      val len14 = pathAdder(len13, valid_node_neighbor_1994)
      checkIt(len14)

      ixx = ixx + 1
      val len15 = pathAdder(len14, valid_node_neighbor_1994)
      checkIt(len15)

      ///////////////////////////////////////////////////

      ixx = ixx + 1
      val len16 = pathAdder(len15, valid_node_neighbor_1994)
      checkIt(len16)

      ixx = ixx + 1
      val len17 = pathAdder(len16, valid_node_neighbor_1994)
      checkIt(len17)

      ixx = ixx + 1
      val len18 = pathAdder(len17, valid_node_neighbor_1994)
      checkIt(len18)

      ixx = ixx + 1
      val len19 = pathAdder(len18, valid_node_neighbor_1994)
      checkIt(len19)

      ixx = ixx + 1
      val len20 = pathAdder(len19, valid_node_neighbor_1994)
      checkIt(len20)
    } catch {
